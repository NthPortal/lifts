...
object LiftKind1 extends LowPriorityLiftKind1Implicits {
  ...
  private[this] final class Identity[F[_]]
      extends LiftKind1[F, F]
      with LiftValue.Identity[F]
      with MapK.Identity[F, F]
  ...
  private object Composed {
    def apply[From[_], Middle[_], To[_]](
      ...
      if (inner.isInstanceOf[Identity[From]]) outer.asInstanceOf[LiftKind1[From, To]]
      else if (outer.isInstanceOf[Identity[Middle]]) inner.asInstanceOf[LiftKind1[From, To]]
      else new Composed(inner, outer)
  }

  implicit def id[F[_]]: LiftKind1[F, F] = _identity.asInstanceOf[Identity[F]]
